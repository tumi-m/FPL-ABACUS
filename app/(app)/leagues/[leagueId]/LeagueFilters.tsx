...
/**
 * Prize-condition filter rail for league standings. Every control writes into
...
export function LeagueFilters({ basePath, disabled }: { basePath: string; disabled?: boolean }) {
  ...
  return (
    <form onSubmit={apply} className="flex flex-wrap items-end gap-2" aria-label="Filter standings">
      ...
      {(q || minGw || topN || params.get("q") || params.get("minGw") || params.get("topN")) && (
        <button
          type="button"
          onClick={() => {
            setQ("");
